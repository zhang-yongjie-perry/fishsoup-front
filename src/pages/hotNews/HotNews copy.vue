<template>
    <Container>
        <a-card :loading="loading0">
            <template #title>
                <span>微博热搜</span>
			</template>
            <a-list :grid="{ gutter: 120, xs: 1, sm: 2, md: 2 , lg: 2, xl: 2, xxl: 2, xxxl: 2 }" :data-source="weiboNews" :locale="{ emptyText: '暂无数据' }">
                <template #loadMore>
                    <div v-if="weiboNewsHasMore" :style="{ textAlign: 'center', marginTop: '12px', height: '32px', lineHeight: '32px' }">
                        <a-spin v-if="weiboNewsLoadingMore" />
                        <a-button v-else @click="toGetWeiboNewsList">加载更多</a-button>
                    </div>
                </template>
...
function toGetWeiboNewsList() {
    listHotNews({time: today_format, title: searchTextState.getSearchText(), site: "weibo", newsType: "1"}, featuredNewsPage.value, 10)
    .then(res => {
        if (res.data.code === '1') {
            warningAlert(res.data.msg)
            return
